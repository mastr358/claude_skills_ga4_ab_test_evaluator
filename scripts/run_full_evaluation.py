...
def generate_html_overview(
    ...
    # Helper for p-value color (gradient from red > 0.15 to green <= 0.05)
    def pvalue_class(pvalue: float) -> str:
        if pvalue <= 0.05:
            return 'pval-green'      # Most green - significant
        elif pvalue <= 0.07:
            return 'pval-green-light'
        elif pvalue <= 0.10:
            return 'pval-yellow-green'
        elif pvalue <= 0.12:
            return 'pval-yellow'
        elif pvalue <= 0.15:
            return 'pval-orange'
        else:
            return 'pval-red'        # Red - not significant
...
<head>
    ...
    <style>
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        ...
        .pval-green-light {{ background-color: #558b2f; }}
        .pval-yellow-green {{ background-color: #9e9d24; }}
        .pval-yellow {{ background-color: #f9a825; }}
        .pval-orange {{ background-color: #ef6c00; }}
        .pval-red {{ background-color: #c62828; }}
...
<div class="container">
    ...
    <p class="note">Data: {start_date} to {end_date} | {days} days | Generated by Martin Štrbák's Claude Code</p>
</div>
</body>
</html>'''
